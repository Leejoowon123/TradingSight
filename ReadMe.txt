exrpess.js를 통해 웹 개발을 하기 앞서 알아야할 정보들
...
토큰 = 토큰은 클라이언트 측에서 상태를 유지하기 위해 사용되는 인증 메커니즘입니다. 주로 JSON Web Token (JWT)이 사용됩니다. 
      토큰은 서버에서 생성되어 클라이언트에게 전달되고, 클라이언트는 이후 요청마다 이 토큰을 서버로 전송하여 인증을 수행합니다.

세션 = 세션은 서버 측에서 사용자 상태를 유지하기 위해 사용되는 데이터 저장소입니다. 
      사용자가 웹사이트를 방문하고 브라우저를 닫거나 로그아웃할 때까지 지속됩니다.
